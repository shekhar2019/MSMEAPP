...
namespace Portal.Core
{
    public class ProductMainGroupBL
    {
        ...
        public ResponseOut AddEditProductMainGroup(ProductMainGroupViewModel productmaingroupViewModel)
        {
            ...
            try
            {
               ProductMainGroup productmaingroup = new ProductMainGroup
               {
                   ...
                   ProductMainGroupDesc=productmaingroupViewModel.ProductMainGroupDesc,
                ...
                if (dtProductMainGroups != null && dtProductMainGroups.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtProductMainGroups.Rows)
                    {
                        productmaingroups.Add(new ProductMainGroupViewModel
                        {
                            ...
                            ProductMainGroupDesc=Convert.ToString(dr["ProductMainGroupDesc"]),
                            ProductMainGroupImageUrl=Convert.ToString(dr["ProductMainGroupImageUrl"]),
        ...
        public List<ProductMainGroupViewModel> GetProductMainGroupList()
        {
            ...
            try
            {
                ...
                if (productMainGroupList != null && productMainGroupList.Count > 0)
                {
                    ...
                    {
                        productMainGroups.Add(new ProductMainGroupViewModel {
                            ProductMainGroupId = productMainGroup.ProductMainGroupId,
                            ProductMainGroupName = productMainGroup.ProductMainGroupName
                        });
                    }
                }
            }
        ...
        public ResponseOut UpdateProductMainGroupPic(ProductMainGroupViewModel productMainGroupViewModel)
        {
            ResponseOut responseOut = new ResponseOut();
            try
            {
                ...
                {
                    ProductMainGroupId= productMainGroupViewModel.ProductMainGroupId,
                    ProductMainGroupImageUrl= productMainGroupViewModel.ProductMainGroupImageUrl
                };
                responseOut = dbInterface.UpdateProductMainGroupPic(productMainGroup);
            }
            catch (Exception ex)
            {
                responseOut.status = ActionStatus.Fail;
