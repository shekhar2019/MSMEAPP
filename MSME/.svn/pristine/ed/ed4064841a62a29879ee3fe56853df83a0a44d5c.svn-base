...
namespace Portal.Core
{
    public class BookBL
    {
        ...
        public List<BookViewModel> GetBookList(string bookName = "", string bookType = "0", string bookCode = "", int companyId = 0, string status = "")
        {
            List<BookViewModel> books = new List<BookViewModel>();
            SQLDbInterface sqlDbInterface = new SQLDbInterface();
            try
            {
                DataTable dtBooks = sqlDbInterface.GetBookList(bookName, bookType, bookCode, companyId, status);
        ...
        public List<BookViewModel> GetBookAutoCompleteList(string searchTerm, string bookType, int companyId)
        {
            List<BookViewModel> books = new List<BookViewModel>();
            try
            {
                List<Book> bookList = dbInterface.GetBookAutoCompleteList(searchTerm, bookType, companyId);
        ...
        public List<BookViewModel> GetBookList(string bookType, int companyId)
        {
            List<BookViewModel> bookList = new List<BookViewModel>();
            try
            {
                List<Portal.DAL.Book> books = dbInterface.GetBookList(bookType,companyId);
                if (books != null && books.Count > 0)
                {
