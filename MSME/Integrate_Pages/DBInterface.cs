...
namespace Portal.DAL
{
    ...
    public partial class DBInterface : IDisposable
    {
        ...
        public List<SellerProductDetailList> GetProductBySellerId(long SellerId)
        {
            var SellerProductsCategory = (from bp in entities.SellerProducts
                                          join psg in entities.ProductSubGroups on bp.CategoryId equals psg.ProductSubGroupId
                                          join pscg in entities.ProductSubChildGroups on bp.SubCategoryId equals pscg.ProductSubChildGroupId
                                          join p in entities.Products on bp.ProductId equals p.Productid
                                          join u in entities.UOMs on bp.UomId equals u.UOMId
                                          where bp.SellerId == SellerId
                                          select new
                                          {
                                              bp.SellerProductDetailId,
                                              bp.ParentClassId,
                                              bp.CategoryId,
                                              bp.SubCategoryId,
                                              bp.ProductId,
            ...
            try
            {
                ...
                else
                {
                    if (userRegistration.UserId == 0)
                    {
                        ...
                        responseOut.message = ActionMessage.UserRegistrationSuccess;
                    }
                    else
                    {
        ...
        public UserRegistration AuthenticateBSUser(string userName, string password)
        {
            ...
            try
            {
                if (entities.UserRegistrations.Any(x => x.UserName == userName && x.Password == password && x.UserStatus == true))
                {
                    userRegistration = entities.UserRegistrations.Where(u => u.UserName == userName && u.Password == password && u.UserStatus == true).FirstOrDefault();
                }
            }
            catch (Exception ex)
            {
        ...
        public ResponseOut AddUpdateBuyerProduct(BuyerProduct buyerProduct)
        {
            ResponseOut responseOut = new ResponseOut();
            try
            {
        ...
        public ResponseOut AddUpdateSellerProduct(SellerProduct sellerProduct)
        {
            ResponseOut responseOut = new ResponseOut();
            try
            {
