...
namespace Portal.Core
{
    public class LeadSourceBL
    {
        ...
        public List<LeadSourceViewModel> GetLeadSourceList(string leadsourceName = "", string Status = "", int companyId = 0)
        {
            List<LeadSourceViewModel> leadsources = new List<LeadSourceViewModel>();
            SQLDbInterface sqlDbInterface = new SQLDbInterface();
            try
            {
                DataTable dtLeadSources = sqlDbInterface.GetLeadSourceList(leadsourceName, Status, companyId);
