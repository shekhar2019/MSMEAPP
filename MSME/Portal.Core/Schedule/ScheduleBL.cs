...
namespace Portal.Core
{
    public class ScheduleBL
    {
        ...
        public List<ScheduleViewModel> GetScheduleList(string scheduleName = "", int companyId=0, string status = "")
        {
            List<ScheduleViewModel> scheduleViewModel = new List<ScheduleViewModel>();      
            SQLDbInterface sqlDbInterface = new SQLDbInterface();
            try
            { 
                DataTable dtSchedules = sqlDbInterface.GetScheduleList(scheduleName, companyId, status);
