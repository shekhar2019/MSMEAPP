...
namespace Portal.Core
{
    public class ProductSubGroupBL
    {
        ...
        public ResponseOut AddEditProductSubGroup(ProductSubGroupViewModel productsubgroupViewModel)
        {
            ...
            try
            {
                ProductSubGroup productsubgroup = new ProductSubGroup
                {
                    ...
                    ProductSubGroupDesc= productsubgroupViewModel.ProductSubGroupDesc,
                    ProductSubGroupImageUrl= productsubgroupViewModel.ProductSubGroupImageUrl,
                ...
                if (dtProductSubGroups != null && dtProductSubGroups.Rows.Count > 0)
                {
                    foreach (DataRow dr in dtProductSubGroups.Rows)
                    {
                        productsubgroups.Add(new ProductSubGroupViewModel
                        {
                            ...
                            ProductSubGroupDesc=Convert.ToString(dr["ProductSubGroupDesc"]),
                            ProductSubGroupImageUrl=Convert.ToString(dr["ProductSubGroupImageUrl"]),
        ...
        public ResponseOut UpdateProductSubGroupPic(ProductSubGroupViewModel productSubGroupViewModel)
        {
            ResponseOut responseOut = new ResponseOut();
            try
            {
                ...
                responseOut = dbInterface.UpdateProductSubGroupPic(productSubGroup);
            }
            catch (Exception ex)
            {
                responseOut.status = ActionStatus.Fail;
