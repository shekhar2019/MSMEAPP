...
namespace Portal.Core
{
    public class LeadBL
    {
        ...
        public List<LeadViewModel> GetLeadList(string leadCode = "", string companyName = "", string contactPersonName = "",  string email = "", string contactNo = "", string companyAddress = "", string companyCity = "", int companyStateId = 0,  int leadStatusId = 0, int leadSourceId = 0,int createdId=0,int userId=0 ,string status = "")
        {
            List<LeadViewModel> leads = new List<LeadViewModel>();
            SQLDbInterface sqlDbInterface = new SQLDbInterface();
            try
            {
                DataTable dtLeads = sqlDbInterface.GetLeadList(leadCode, companyName, contactPersonName, email,  contactNo, companyAddress, companyCity, companyStateId, leadStatusId, leadSourceId,createdId, userId, status);
